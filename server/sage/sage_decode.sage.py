...
# This file was *autogenerated* from the file sage_decode.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_8 = Integer(8)
import json
...
def handle_task(task):
    ...
    if "from_str" in task:
        ...
        as_obj = as_str[_sage_const_2 :]  # beg. contains inform. on code
        cc = ConcatenatedCode.from_file(as_obj)
        msg = vector(GF(_sage_const_2 ), task["message"])
        # this could be more than decode call
        out = []
        bl_len = cc.length()
        for i in range(_sage_const_0 , len(msg), bl_len):
            tmps = cc.decode(msg[i:i+bl_len]).list()
            out += tmps
        # change to bitstream
        bit_str = []
        f_size = int(round(math.log(cc._of.order(), _sage_const_2 )))
        for wo in out:
            icomp = wo.to_integer()
            for l in range(f_size):
                tmpz = (icomp >> l) & _sage_const_1 
                bit_str.append(tmpz)

        # re-combine
        to_send = []
        mc = _sage_const_0 
        up_b = len(bit_str) // _sage_const_8 
        for i in range(_sage_const_0 , up_b*_sage_const_8 , _sage_const_8 ):
            to_int = _sage_const_0 
            for j in range(_sage_const_8 ):
                to_int += bit_str[i+j]*_sage_const_2 **j
            to_send.append(int(to_int))
        # for i in range(len(out)):
        #    to_send.append(out[i].to_integer())
    else:
        # outer code
        n = task["outer"]["parameters"][_sage_const_0 ]
        k = task["outer"]["parameters"][_sage_const_1 ]
        q = task["outer"]["parameters"][_sage_const_2 ]
        ...
        if task["inner"]["type"] == 'ReedMuller':
            r = task["inner"]["parameters"][_sage_const_0 ]  # order
            m = task["inner"]["parameters"][_sage_const_1 ]  # number of variables
            inner_n = _sage_const_2 **m
            inner_k = m + _sage_const_1 
            inner_C = codes.BinaryReedMullerCode(r, m)

        # inner code -- Hamming
        if task["inner"]["type"] == 'Hamming':
            inner_n = _sage_const_2  ** task["inner"]["parameters"][_sage_const_0 ] - _sage_const_1 
            inner_k = _sage_const_2  ** task["inner"]["parameters"][_sage_const_0 ] -                 task["inner"]["parameters"][_sage_const_0 ] - _sage_const_1 
            inner_C = codes.HammingCode(GF(_sage_const_2 ), task["inner"]["parameters"][_sage_const_0 ])

        to_decode = vector(GF(_sage_const_2 ), task["message"])

        inner_D = inner_C.decoder("NearestNeighbor")

        inner_decoded = []
        for i in range(_sage_const_0 , len(to_decode), inner_n):
            to_decode_partial = vector(GF(_sage_const_2 ), to_decode[i:i+(inner_n)])
        ...
        from_inner = [F(inner_decoded[i:i+_sage_const_8 ])
                      for i in range(_sage_const_0 , len(inner_decoded), _sage_const_8 )]
