...
### 端到端加密（E2EE）
- **对称助记词方案**：双方线下约定各自的助记词（任意字符串），客户端 PBKDF2 派生 `user_key`，组合双方的 `user_key` 经 HKDF 派生对称 `mkey`
- **AES-256-GCM 加密消息**：普通文本聊天、隐写消息正文全部加密，服务器永远看不到明文
- **隐写 seed 异或派生**：chat 模型 seed 由双方邀请码按字节 XOR 得到，双方算出同一值共享后端模型
- **指纹校对**：每个 `user_key` 展示 8 字节 SHA-256 指纹，便于双方当面比对
- **向后兼容**：升级前的明文消息历史不迁移不解密，原样展示
- 详细协议参数见 [docs/superpowers/specs/2026-05-10-e2e-encrypted-chat-design.md](docs/superpowers/specs/2026-05-10-e2e-encrypted-chat-design.md)
...
服务器与邀请码默认是公开的，但消息正文可以通过双方**线下约定的助记词**做 E2E 加密，服务器无法解读。启用步骤：

1. **双方线下约定各自的助记词**（任意字符串，例如一句话或随机组合），双方都要分别告诉对方自己的助记词
2. **Web 或 Android 的"设置"页**：填入自己的助记词 → 保存，界面会显示 8 字节指纹，可与对方当面核对
3. **"联系人详情"页**：为每个联系人单独填入对方告诉你的助记词 → 保存，界面会显示对方指纹
4. 双方都完成上述两步后，该联系人的聊天（普通文本 + 隐写消息）自动切换为 E2EE，输入框启用

**注意**：
- 助记词与派生密钥明文存储于本地 DB（Web IndexedDB / Android Room），不做"at rest 加密"，属于论文 demo 级简化
- 助记词丢失无法恢复历史消息（与 E2E 语义一致：服务端不托管任何密钥）
- 重置助记词会让之前加密的消息无法解密
- 详细密码学参数（PBKDF2 salt / iteration、HKDF info、GCM 参数）见 [docs/superpowers/specs/2026-05-10-e2e-encrypted-chat-design.md](docs/superpowers/specs/2026-05-10-e2e-encrypted-chat-design.md) §4
...
| 聊天输入框灰色无法输入 | E2EE 未配置完整 | 按 "启用端到端加密" 配自己的 + 对方的助记词 |
| 聊天消息显示 `[无法解密]` | 双方助记词不一致 / 对方未配你的助记词 | 当面比对指纹,或重置对方助记词后重填 |
| 开了隐写模式但提示容量不足 | sealed 比明文大约 37%,界面显示的容量已打 0.65× 折扣 | 缩短明文,或独立隐写工具页不受 E2EE 影响 |
